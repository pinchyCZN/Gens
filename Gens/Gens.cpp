...
extern HWND hTrainer;
	if(main68k_context.watchhit!=0)
	{
	sprintf(text,"Address:0x%08X\r\nValue:%i",main68k_context.watchhit,main68k_context.watchvalue);
	main68k_context.watchhit=0;
#include "resource.h"
	SetDlgItemText(hTrainer,IDC_WATCH,text);
	}
	if(sub68k_context.watchhit!=0)
	{
	sprintf(text,"Address:0x%08X\r\nValue:%i",sub68k_context.watchhit,sub68k_context.watchvalue);
	sub68k_context.watchhit=0;
	SetDlgItemText(hTrainer,IDC_WATCH,text);
	}
	return(1);
}
