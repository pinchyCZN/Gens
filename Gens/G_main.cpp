...
static ULONG getdata(BYTE *buffer,int size)
{
	int i,j=0;
	for(i=(size-1);i>=0;i--)
	{
...
HWND hTrainer=0;
int		found=0;
BOOL CALLBACK Trainer(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
		int				i,j;
		unsigned char	*ram;
		...
		static int		size=1;
		static int		trainer_value=0;
		static BOOL		rom_space=FALSE;
		static int		which_cpu=0;

		switch (message)
		{
		case WM_INITDIALOG :
			buffer=(ULONG*)malloc(64*1024*8);
			if(buffer==0)
				EndDialog( hDlg, 0 );
			return TRUE;
		case WM_COMMAND:
			switch (LOWORD(wParam))
			{
				...
				case IDC_TSIZE:
					...
					SetDlgItemText(hTrainer,IDC_SIZETEXT,text);
					break;
				case IDC_SEARCH:
					found=0;
					memset(buffer,0,64*1024*8);
					SendDlgItemMessage(hTrainer,IDC_MATCHES, LB_RESETCONTENT,0,0); //clear  all
					GetDlgItemText(hTrainer,IDC_VALUE,text,10);
					if(which_cpu==0)
						ram=(BYTE*)Ram_68k;
					else
						ram=(BYTE*)Ram_Prg;
					if(text[0]==0)
					{
						for(i=0;i<(64*1024);i+=size)
						{
							buffer[i*2]=i;
							buffer[i*2+1]=getdata(ram+i,size);
					...
					{
						trainer_value=strtoul(text,NULL,0);
						for(i=0,j=0;i<(64*1024);i+=size)
						{
							if(trainer_value==getdata(ram+i,size))
							{
							buffer[j*2]=i;
							buffer[j*2+1]=trainer_value;
							j++;
							found=j;
							}
						}
					...
					for(i=0;i<((found>80)?80:found);i++)
					{
						if((buffer[i*2]!=0) || (i==0))
						{
							sprintf(text,"0x%04X 0x%02X %i",buffer[i*2],buffer[i*2+1],buffer[i*2+1]);
							SendDlgItemMessage(hTrainer,IDC_MATCHES, LB_INSERTSTRING,i,(LONG)text);
						}
					}
					break;
				case IDC_NEXT:
					...
					else
					{
						...
						{
							if(trainer_value==getdata(ram+buffer[i*2],size))
							{
				...
				case IDC_SAME:
					...
					for(i=0,j=0;i<found;i++)
					{
						if(buffer[i*2+1]==getdata(ram+buffer[i*2],size))
						{
						buffer[j*2]=buffer[i*2];
						buffer[j*2+1]=buffer[i*2+1];
						j++;
						}
					}
					found=j;
				...
				case IDC_DIFFERENT:
					...
					for(i=0,j=0;i<found;i++)
					{
						...
						{
						buffer[j*2]=buffer[i*2];
						buffer[j*2+1]=getdata(ram+buffer[i*2],size);
						j++;
						}
					}
					found=j;
				...
				case IDC_LESS:
					...
					break;
				case IDC_ADDLIST:
					i=SendDlgItemMessage(hTrainer,IDC_MATCHES,LB_GETSELITEMS,100,(LONG)temp);
					if(i==0)
						break;
					for(j=0;j<i;j++)
					{
					SendDlgItemMessage(hTrainer,IDC_MATCHES, LB_GETTEXT,temp[j],(LONG)text);
					SendDlgItemMessage(hTrainer,IDC_TRAIN_LIST, LB_INSERTSTRING,j,(LONG)text);
					}
					break;
				case IDC_CLEARLIST:
					SendDlgItemMessage(hTrainer,IDC_TRAIN_LIST, LB_RESETCONTENT,0,0); //clear  all
					main68k_context.dowatch=0;
					break;
				case IDC_TRAIN_LIST:
					if (HIWORD(wParam)==LBN_SELCHANGE)
					{
						i=SendDlgItemMessage(hTrainer,IDC_TRAIN_LIST,LB_GETCURSEL,0,0);
						if (i!=LB_ERR)
						{
						SendDlgItemMessage(hTrainer,IDC_TRAIN_LIST,LB_GETTEXT,i,(LONG)text);
						if(which_cpu==0){
						main68k_context.watchpoint=strtoul(text,NULL,16);
						main68k_context.dowatch=1;
						}
						else{
						sub68k_context.watchpoint=strtoul(text,NULL,16);
						sub68k_context.dowatch=1;
						}
						...
						for(i=0;i<25;i++)
						{
							...
							{
								text[i]=0;
								SetDlgItemText(hTrainer,IDC_CURRENTADDRESS,text);
								for(i++,j=i;i<25;i++)
								{
									if(text[i]==0x20)
									{
									text[i]=0;
									temp[0]=strtoul(text+j,NULL,0);
									if(temp[0]>9999)
									SetDlgItemText(hTrainer,IDC_CURRENTVALUE,text+j);
									else
									SetDlgItemInt(hTrainer,IDC_CURRENTVALUE,temp[0],FALSE);
									break;
									}
								}
								break;
						...
						if(which_cpu==0)
						sprintf(text,"watching address:0x%04X",main68k_context.watchpoint);
						else
						sprintf(text,"watching address:0x%04X",sub68k_context.watchpoint);

						SetDlgItemText(hTrainer,IDC_WATCH,text);
				...
				if(found!=0)
				{
					...
					for(i=0;i<((found>80)?80:found);i++)
					{
					buffer[i*2+1]=getdata(ram+buffer[i*2],size);
					sprintf(text,"0x%04X 0x%02X %i",buffer[i*2],buffer[i*2+1],buffer[i*2+1]);
					SendDlgItemMessage(hTrainer,IDC_MATCHES, LB_INSERTSTRING,i,(LONG)text);
					}
				}
				break;
				case IDC_UPDATE:
					GetDlgItemText(hTrainer,IDC_CURRENTADDRESS,text,12);
					temp[0]=strtoul(text,NULL,16);
					GetDlgItemText(hTrainer,IDC_CURRENTVALUE,text,12);
					temp[1]=strtoul(text,NULL,0);
					i=SendDlgItemMessage(hTrainer,IDC_TRAIN_LIST,LB_GETCURSEL,0,0);
					if (i!=LB_ERR)
					{
						sprintf(text,"0x%04X 0x%04X %i",temp[0],temp[1],temp[1]);
						SendDlgItemMessage(hTrainer,IDC_TRAIN_LIST, LB_DELETESTRING,i,NULL);
						SendDlgItemMessage(hTrainer,IDC_TRAIN_LIST,LB_INSERTSTRING,i,(LONG)text);
						SendDlgItemMessage(hTrainer,IDC_TRAIN_LIST,LB_SETCURSEL,i,NULL);
					}
					else{
						if(which_cpu==0)
							ram=(BYTE*)Ram_68k;
				...
				case IDC_ADDCHEAT:
					...
					{
						SendDlgItemMessage(hTrainer,IDC_TRAIN_LIST,LB_GETTEXT,i,(LONG)text);
						j=strtoul(text,NULL,0);
						if(rom_space)
						Liste_GG[250].addr=j;
						else if(which_cpu==0)
						Liste_GG[250].addr=0xFF0000|j;
						else
						Liste_GG[250].addr=0xEF000000|j;
						for(i=0;i<25;i++)
						{
							if(text[i]==0x20)
							{
								j=strtoul(text+i+1,NULL,0);
								//Liste_GG[250].data=((j>>8)&0xFF)|((j<<8)&0xFF00);
								Liste_GG[250].data=j&0xFFFF;
								break;
							}
						}
						sprintf(Liste_GG[250].code,"%08X",Liste_GG[250].addr);
						Liste_GG[250].active=TRUE;
						strcpy(Liste_GG[250].name,"Test");
					}
					break;
				case IDC_REMOVECHEAT:
					memset(&Liste_GG[250],0,sizeof(GG_Code));
					break;
				case IDC_RAMROM:
					if (HIWORD(wParam)==BN_CLICKED)
					{
						if(!rom_space){
							SendDlgItemMessage(hTrainer,IDC_RAMROM,BM_SETCHECK,BST_CHECKED,0);
							rom_space=TRUE;}
						else{
							SendDlgItemMessage(hTrainer,IDC_RAMROM,BM_SETCHECK,BST_UNCHECKED,0);
							rom_space=FALSE;}
					}
					break;
				case IDC_WHICHCPUT:
					...
					{
					case 0:
						SetDlgItemText(hDlg,IDC_WHICHCPUT,"M68k");
						break;
					case 1:
						SetDlgItemText(hDlg,IDC_WHICHCPUT,"Slave68k");
						break;
					case 2:
						SetDlgItemText(hDlg,IDC_WHICHCPUT,"Z80");
						break;
					}
					break;
	...
	switch(message)
	{
		...
		case WM_COMMAND:
			...
			else switch(LOWORD(wParam))
			{
				...
				case ID_TRAINER:
					if(hTrainer==0)
					hTrainer = CreateDialog (NULL, MAKEINTRESOURCE(IDD_TRAINER), 
                              hWnd, Trainer) ;
					//hTrainer=(HWND)GetLastError();
					break;
#ifdef GENS_DEBUG
				case ID_CPU_DEBUG_GENESIS_68000:
					extern HWND hDebugger;
					extern BOOL CALLBACK Debugger(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam);
					if(hDebugger==0)
					hDebugger = CreateDialog (NULL, MAKEINTRESOURCE(IDD_DEBUGGER), 
                              hWnd, Debugger) ;
					//Change_Debug(hWnd, 1);
					break;
...
HMENU Build_Main_Menu(void)
{
	...
	MENU_L(CPU, i++, Flags, ID_TRAINER, "TRAINER", "", "&TRAINER");
...
LRESULT CALLBACK GGenieProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		case WM_INITDIALOG:
			//Associate the spin control with the static text
			SetDlgItemInt(hDlg,IDC_CURRENTCODE,(UINT)current_code,FALSE);
			
			SendMessage(GetDlgItem (hDlg,IDC_CODESELECT),UDM_SETBUDDY,
						(LONG)GetDlgItem (hDlg,IDC_CURRENTCODE),0L);

			SendMessage(GetDlgItem (hDlg,IDC_CODESELECT),UDM_SETRANGE,
						0L,MAKELONG(255,0));
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case IDC_CURRENTCODE:
					if(HIWORD(wParam)==EN_CHANGE)
					{
					current_code=GetDlgItemInt(hDlg,IDC_CURRENTCODE,NULL,FALSE);
					if(Liste_GG[current_code].addr==-1)
						Str_Tmp[0]=0;
					else
					sprintf(Str_Tmp,"%06.6X:%04.4X",Liste_GG[current_code].addr,Liste_GG[current_code].data);
					SetDlgItemText(hDlg,IDC_EDIT1,Str_Tmp);
					SetDlgItemText(hDlg,IDC_EDIT2,Liste_GG[current_code].name);
					}
					break;
				case ID_EDITCODE:
					//same as add code cept for insert string instead of add string
				case ID_GGADD:
					if (GetDlgItemText(hDlg, IDC_EDIT1, Str_Tmp, 14))
					{
						...
						{						
							...
							if(LOWORD(wParam)==ID_GGADD)
							SendDlgItemMessage(hDlg, IDC_LIST1, LB_ADDSTRING, (WPARAM) 0, (LONG) (LPTSTR) Str_Tmp);
							else
							{
							SendDlgItemMessage(hDlg, IDC_LIST1, LB_DELETESTRING, (WPARAM) current_code, NULL);
							SendDlgItemMessage(hDlg, IDC_LIST1, LB_INSERTSTRING, (WPARAM) current_code, (LONG) (LPTSTR) Str_Tmp);
							}
