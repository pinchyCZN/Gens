...
unsigned char MDscreen32[336*240*4+320*32*2]; //orig 336*240
...
int Init_DDraw(HWND hWnd)
{
	...
	if (Full_Screen)
	{
		if (lpDD->SetDisplayMode(320 * ((Rend > 0)?2:1), 240 * ((Rend > 0)?2:1), 32, 0, 0) != DD_OK)
	...
	if (Rend < 2)
	{
		...
		ddsd.dwHeight = 240;
		ddsd.lPitch = 336 * 4;
		ddsd.lpSurface = MDscreen32; //&MD_Screen[0];
...
extern HWND hDebugger;
	if(main68k_context.breakpoint!=main68k_context.pc)
		stop=FALSE;
	else
	if(hDebugger && !stop)
	{
		SendMessage(hDebugger,WM_USER,0xFF,0);
		stop=TRUE;
	}
