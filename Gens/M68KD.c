...
char *M68KDisasm(unsigned short (*NW)(), unsigned int (*NL)())
{
	...
	switch(OPC >> 12)
	{
		...
		case 6:
			i=Current_PC+(char)(OPC & 0xFF);
			if (OPC & 0xFF)
			{
				if ((OPC & 0xF00) == 0x100)
				{
					//BSR  label
					sprintf(Dbg_Str, "BSR     #$%.2X%c", i, 0);
					break;
				}
				...
				{
					//BRA  label
					sprintf(Dbg_Str, "BRA     #$%.2X%c", i, 0);
					break;
				}

				//BCC  label

				sprintf(Dbg_Str, "B%-7s %.2X%c", Make_Dbg_Cond_Str((OPC >> 8) & 0xF),i , 0);
			...
			else
			{
				...
				{
					//BRA  label
					sprintf(Dbg_Str, "BRA     %.4X%c", i, 0);
					break;
				}

				//BCC  label
				sprintf(Dbg_Str, "B%-7s %.4X%c", Make_Dbg_Cond_Str((OPC >> 8 ) & 0xF), i, 0);
			}
			break;
