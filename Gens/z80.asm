...
ALIGN32
CHECK_Breakpoint:
	cmp		[ebp+Z80.breakpoint],zxPC
	jne		.skip
	mov		edi,-1
	jmp		near z80_Exec_Quit
.skip
	cmp		dword [ebp+Z80.watchpoint],0
	je		.skip2
	push	eax
	mov		eax,[ebp+Z80.watchpoint]
	movzx	eax,byte [eax]
	cmp		[ebp+Z80.watch_data],eax
	je		.itsame
	mov		eax,[Z80.BasePC]
	neg		eax
	add		eax,zxPC
	mov		[Z80.watchhit],eax
.itsame
	pop		eax
.skip2
	ret
...
%if (GENS_DEBUG=1)
	...
	cmp		dword [ecx + Z80.watchpoint],0
	je		.skipwatch
	mov		eax,[ecx + Z80.watchpoint]
	movzx	eax,byte [eax]
	mov		[ecx + Z80.watch_data],eax
.skipwatch
	mov		eax,[ecx + Z80.breakpoint]
	cmp		eax,0
	je		.skipbreak
	cmp		eax,[ecx + Z80.PC]
	je		near z80_Cycles_Already_done
.skipbreak
%endif
	push ebx
	push edi
