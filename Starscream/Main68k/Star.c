...
static void gen_variables(void) {
	...
	*/
	if(cputype >= 68000) {
		...
		emit("__watchpoint			 dd 0\n");
		emit("__breakpoint			 dd 0\n");
		emit("__dowatch				 dd 0\n");
		emit("__watchvalue			 dd 0\n");
		emit("__watchhit			 dd 0\n");
	}
	/*
	** CONTEXTINFO_68010
	**
...
emit(".notstopped:\n");
	...
	*/
	...
	/*check if we are gona do watchpoints*/

	emit("cmp	dword[__dowatch],1\n");
	emit("jne	.skip\n");
	emit("push edx\n");
	emit("mov	edx,[__watchpoint]\n");
	emit("movzx	edx, byte [Ram_68k + edx]\n");
	emit("mov	[__watchvalue],edx\n");
	emit("pop edx\n");
	emit(".skip:\n");
	emit("cmp	dword[__breakpoint],0\n");
	emit("je	.skip3\n");
	emit("push	eax\n");
	emit("mov	eax,[__breakpoint]\n");
	emit("cmp	eax,[__pc]\n");		//normally it would be esi-ebp
	emit("pop		eax\n");
	emit("jne	.skip3\n");
	emit("mov	edi,0xFFF00000\n");
	emit("jmp execquit_nointerrupt\n");
	emit(".skip3:\n");
...
/*	emit("or byte[__stopped],2\n");*/
	...
	//Watchroutine
	emit("watch_rutine:\n");
	emit("cmp	byte[__dowatch],1\n");
	emit("jne	.skipallthis\n");
	emit("push	ecx\n");
	emit("mov	ecx,[__watchpoint]\n");
	emit("mov	cl, byte [Ram_68k + ecx]\n");
	emit("cmp	cl, [__watchvalue]\n");
	emit("je	.skipthis\n");	//skip if nothing changed
	emit("mov	[__watchvalue],cl\n");
	emit("mov	ecx,esi\n");
	emit("sub	ecx,ebp\n");
	emit("mov	[__watchhit],ecx\n");
	emit("mov	dword[__dowatch],0\n");
	emit(".skipthis:\n");
	emit("pop ecx\n");
	emit(".skipallthis:\n");
	emit("cmp	dword[__breakpoint],0\n"); //check breakpoint now
	emit("je	.skip3\n");
	emit("push	eax\n");
	emit("push	esi\n");
	emit("mov	eax,[__breakpoint]\n");
